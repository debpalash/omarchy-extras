...
- The foundation announcement is a bright editorial hyperlink beneath the ASCII mark, set apart with a fine green underline and measured spacing. It stays noticeable without competing with the mark or reading like a promotional banner.
...
- The About window uses a neofetch-style split: an oversized Omarchy mark and compact identity caption beside terminal-framed hardware, software, and live session readouts. Narrow screens stack the mark above the same information without replacing measured values with mock system data.
...
- Static-page mastheads reserve the launcher's full left footprint and share one vertical centerline, keeping the menu and ASCII wordmark distinct without adding more chrome.
